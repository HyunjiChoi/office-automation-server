...
public class VcMallService {
    ...
    HashMap<String, MoneyData> moneySameList = new HashMap<>();
    HashMap<String, MoneyData> moneyIncPrList = new HashMap<>();
    ...
    HashMap<String, MoneyData> MoneyIncNmCust = new HashMap<>(); // 구매자 불일치
...
/*
    ...
    public void InsertMoneyExcelData(Sheet worksheet) {
        try {
            ...
            for (int i = 1; i < worksheet.getPhysicalNumberOfRows(); i++) {
                ...
                moneyList.put(moneyData.getNmCust(), moneyData);
                ...
                for(int j = 0; j < testSame.size(); j++) {
                    ...
                    if ( testSame.get(keyList.get(j)).getNmCust().equals(moneyData.getNmCust())
                        ...
                        if (vcData.getDpsitTotal() == moneyData.getPrice()) {   // 입금액 = 입금 합계
                            ...
                            moneySameList.put(moneyData.getNmCust(), moneyData);
                            ...
                            moneyIncPrList.put(moneyData.getNmCust(), moneyData);
                        ...
                        MoneyData moneyData2 = moneyList.get(moneyData.getNmCust());
                        MoneyIncNmCust.put(moneyData2.getNmCust(), moneyData2);

                        if(moneySameList.containsKey(moneyData2.getNmCust())
                                ||  moneyIncPrList.containsKey(moneyData2.getNmCust())){
                            MoneyIncNmCust.remove(moneyData2.getNmCust());
                        }
                    }
                }
    ...
    public void downExcelData(HttpServletResponse response) throws IOException {
        ...
        createHeader(sheet1);   // 일치
        createCardHeader(sheet2);   // 주문번호 불일치 (이니시스 형식)
        createHeader(sheet3);   // 주문자 불일치
        createHeader(sheet4);   // 금액 불일치
        ...
        for(int i=0; i<CardIncNoOrder.size(); i++ ) {
            ...
            row2.createCell(1).setCellValue("");    // 상점MID
            row2.createCell(2).setCellValue("");    // 지급일
            row2.createCell(3).setCellValue("");    // 승인일
            row2.createCell(4).setCellValue("");    // 취소일
            row2.createCell(5).setCellValue("");    // 카드일
            row2.createCell(6).setCellValue("");    // 승인번호
            row2.createCell(7).setCellValue("");    // 할부개월수
            row2.createCell(8).setCellValue(CardIncNoOrder.get(CardIncNoKeyList.get(i)).getNoOrder());  // 주문번호
            row2.createCell(9).setCellValue("");    // TID
            row2.createCell(10).setCellValue(CardIncNoOrder.get(CardIncNoKeyList.get(i)).getNmCust());  // 구매자
            row2.createCell(11).setCellValue("");    // 상품명
            row2.createCell(12).setCellValue(CardIncNoOrder.get(CardIncNoKeyList.get(i)).getPrice());   // 거래금액
            row2.createCell(13).setCellValue("");    // M포인트금액
            row2.createCell(14).setCellValue("");    // 수수료
            row2.createCell(15).setCellValue("");    // 무이자수수료
            row2.createCell(16).setCellValue("");    // M포인트수수료
            row2.createCell(17).setCellValue("");    // 부가세
            row2.createCell(18).setCellValue("");    // M포인트부가세
            row2.createCell(19).setCellValue("");    //  지급액
            row2.createCell(20).setCellValue("");    // 에스크로구분
            row2.createCell(21).setCellValue("");    // 상태
        ...
        createHeader(sheet5);       // 일치
        createMoneyHeader(sheet6);  // 주문자 불일치(무통장 형식)
        createHeader(sheet7);       // 금액 불일치
        ...
        for(int j=0; j<MoneyIncNmCust.size(); j++){
            ...
            row6.createCell(1).setCellValue("");    // 계좌번호
            row6.createCell(2).setCellValue("");    // 은행명
            row6.createCell(3).setCellValue("");    // 거래일자
            row6.createCell(4).setCellValue("");    // 거래시간
            row6.createCell(5).setCellValue(MoneyIncNmCust.get(MoneyIncNmKeyList.get(j)).getNmCust());  // 적요/거래자
            row6.createCell(6).setCellValue(MoneyIncNmCust.get(MoneyIncNmKeyList.get(j)).getPrice()); // 입금액
            row6.createCell(7).setCellValue("");    // 출금액
            row6.createCell(8).setCellValue("");    // 잔액
            row6.createCell(9).setCellValue("");    // 거래지점
            row6.createCell(10).setCellValue("");   // 전표처리
            row6.createCell(11).setCellValue("");   // 전표정보
            row6.createCell(12).setCellValue("");   // 데이터코드
    ...
    public Row createCardHeader(Sheet sheet) {
        Row header = sheet.createRow(0);

        header.createCell(0).setCellValue("No");
        header.createCell(1).setCellValue("상점MID");
        header.createCell(2).setCellValue("지급일");
        header.createCell(3).setCellValue("승인일");
        header.createCell(4).setCellValue("취소일");
        header.createCell(5).setCellValue("카드일");
        header.createCell(6).setCellValue("승인번호");
        header.createCell(7).setCellValue("할부개월수");
        header.createCell(8).setCellValue("주문번호");
        header.createCell(9).setCellValue("TID");
        header.createCell(10).setCellValue("구매자");
        header.createCell(11).setCellValue("상품명");
        header.createCell(12).setCellValue("거래금액");
        header.createCell(13).setCellValue("M포인트금액");
        header.createCell(14).setCellValue("수수료");
        header.createCell(15).setCellValue("무이자수수료");
        header.createCell(16).setCellValue("M포인트수수료");
        header.createCell(17).setCellValue("부가세");
        header.createCell(18).setCellValue("M포인트부가세");
        header.createCell(19).setCellValue("지급액");
        header.createCell(20).setCellValue("에스크로구분");
        header.createCell(21).setCellValue("상태");

        return header;
    }

    public Row createMoneyHeader(Sheet sheet) {
        Row header = sheet.createRow(0);

        header.createCell(0).setCellValue("S");
        header.createCell(1).setCellValue("계좌번호");
        header.createCell(2).setCellValue("은행명");
        header.createCell(3).setCellValue("거래일자");
        header.createCell(4).setCellValue("거래시간");
        header.createCell(5).setCellValue("적요/거래자");
        header.createCell(6).setCellValue("입금액");
        header.createCell(7).setCellValue("출금액");
        header.createCell(8).setCellValue("잔액");
        header.createCell(9).setCellValue("거래지점");
        header.createCell(10).setCellValue("전표처리");
        header.createCell(11).setCellValue("전표정보");
        header.createCell(12).setCellValue("데이터코드");
